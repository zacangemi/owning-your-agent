...
| # | Decision | Choice | Why (one line) |
|---|---|---|---|
| 4 | Production model — quality | Qwen3.5-27B (Unsloth UD-Q5_K_XL) | 97% ToolCall-15 at temp 0 (benchmark default) / 100% at temp 0.6 (Qwen-recommended for thinking mode); thinking traces act as a built-in planning layer. Use Qwen for production-quality work where output reliability matters most. |
| 5 | Production model — speed | GLM-4.7-Flash (Unsloth UD-Q5_K_XL) | 2.3–3.4× faster than Qwen; 93% ToolCall-15 (4 points behind Qwen but still production-worthy for most agentic work); no reprocessing bug (pure MLA attention). A good speed alternative when turn-around time matters more than the small accuracy gap. |
...
| Claude-distilled hobbyist fine-tunes (e.g., Jackrong's Qwen3.5-Claude-4.6-Opus-Reasoning-Distilled series) | Hobbyist Claude-distilled models on HuggingFace are fine-tunes of base Qwen/Llama on Claude reasoning outputs — optimized for reasoning *brevity* (shorter CoT chains), not for tool calling. For agentic work, the base Qwen3.5-27B preserves the careful step-by-step tool-call reasoning the model was originally trained for. **Important distinction:** these hobbyist fine-tunes ≠ full lab-distillation pipelines (Z.ai's GLM family, Moonshot's Kimi K2, DeepSeek's training process), which use teacher-student training plus serious RLHF/RLAIF and produce competitive frontier-grade models. We avoided the hobbyist variants on consumer hardware. |
...
| 22 | Hermes auto-compaction threshold | 0.85 (Hermes default — kept) | Hermes auto-compacts at 85% of `context_length` by default. With our 96K window that fires at 98,304 × 0.85 = 83,558 tokens. Hermes then summarizes middle turns into the `summary_model` (we point this at the local model itself — no cloud round-trip), drops context back to a smaller working set, and continues. Configurable via `compression.threshold` in `~/.hermes/config.yaml`; we kept the default. The ~15% headroom between the trigger and the hard context limit gives the agent room to finish its current tool-call round without truncation before compaction kicks in. |
| 23 | TDR (Timeout Detection and Recovery) | Disabled (`TdrLevel=0`) | Default 2-second timeout causes Windows-killing cascade on multi-GPU NCCL cleanup |

## Agent Setup

| # | Decision | Choice | Why (one line) |
|---|---|---|---|
| 24 | System prompt philosophy | Karpathy approach — shape thinking, not tools | SOUL.md = how to think; skills = how to use specific tools; model = which tool to pick |
| 25 | Web search backend | Tavily (native Hermes integration) | DuckDuckGo rate-limited + low quality; browser hits CAPTCHAs; Tavily free tier (1,000/mo) works reliably |
| 26 | Mobile access | Telegram gateway (bot via @BotFather) | 2-minute setup; full Hermes access from anywhere; sovereignty extends past the desk |
| 27 | Approval timeout | 300 seconds (was 60 default) | Long enough to review complex terminal commands without auto-timeout; mode stays manual (no auto-approve) |
| 28 | Streaming | Enabled | UX improvement only — no speed change; visible token output during generation |

## Things we did NOT do

| # | Decision | Choice | Why (one line) |
|---|---|---|---|
| 29 | No `--cache-reuse` flag | (rejected) | Hybrid DeltaNet architecture cannot do partial KV cache reuse; llama.cpp logs warning and ignores |
| 30 | No `--grammar` constraints on GLM | (rejected) | Open issue #19068 — infinite loop with tool calling; use `--jinja` autoparser instead |
| 31 | No reasoning budget cap | (rejected) | Thinking traces enable tool-call accuracy; capping them hurts the primary use case |
| 32 | No live `/model` switching during sessions | (offered by Hermes, not used) | Hermes supports live `/model` swapping mid-session; we never used it in production. We restart sessions with the appropriate model for the task instead. |
